...
#     print("\n" + "❓" * 30)
#     print(f"🎯 CLEAN ISOLATED INSTRUCTOR QUESTION RESOLVED:\n'{instructor_prompt}'")
#     print("❓" * 30 + "\n")
#     return instructor_prompt
# async def run_ai_automation():
#     knowledge = load_knowledge_base()
...
#                 # ✅ FIX: Uses Playwright's native Python selector engine. ZERO JavaScript injection syntax risks.
#                 messages_data = []
#                 try:
...
#                 for msg in messages_data:
#                     if not msg or any(x in msg for x in ["Student", "MUHAMMAD ASJAD", "Understood. Let's focus"]):
#                         continue
    ...
    if os.path.exists(KNOWLEDGE_FILE):
        try:
            with open(KNOWLEDGE_FILE, 'r') as f:
                ...
                if "completed_topics" not in data:
                    data["completed_topics"] = {}
                if "completed_subjects" not in data:
                    data["completed_subjects"] = []
                return data
        except Exception:
            pass
    return {"completed_topics": {}, "completed_subjects": []}
...
# ✅ NEW: Automated Dynamic Link Selector with Duplicate Protection
if not os.path.exists(LINKS_FILE):
    print(f"❌ Error: Required links file '{LINKS_FILE}' not found in root.")
    sys.exit(1)

with open(LINKS_FILE, 'r') as f:
    ALL_SOURCE_LINKS = [line.strip() for line in f.readlines() if line.strip() and "academics" in line]

if not ALL_SOURCE_LINKS:
    print(f"❌ Error: Your '{LINKS_FILE}' does not contain any valid URLs.")
    sys.exit(1)

knowledge = load_knowledge_base()

# Filter out links that have already had all their threads fully handled
ELIGIBLE_POOL = [link for link in ALL_SOURCE_LINKS if link not in knowledge.get("completed_subjects", [])]

# Fallback: Reset the exclusion lock list array if all links have been completely processed
if not ELIGIBLE_POOL:
    print("✨ All links in list file completed. Resetting subject pool baseline tracking fields...")
    knowledge["completed_subjects"] = []
    ELIGIBLE_POOL = ALL_SOURCE_LINKS

TARGET_URL = random.choice(ELIGIBLE_POOL)
print("\n" + "🎲" * 30)
print(f"🎯 POOL MATRIX SUCCESS: Selected 1 URL out of {len(ELIGIBLE_POOL)} available options.")
print(f"👉 TARGET URL LOCATION: '{TARGET_URL}'")
print("🎲" * 30 + "\n")
...
async def run_ai_automation():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            uncompleted_in_subject = 0
            for check_idx in range(count):
                check_text = await topic_locators.nth(check_idx).inner_text()
                check_lines = [l.strip() for l in check_text.split('\n') if l.strip()]
                if check_lines and check_lines not in knowledge["completed_topics"][subject_code]:
                    if not any(x in check_lines for x in ["Discussion topics", "Class conversation", "Chat", "Home", "Dashboard"]):
                        uncompleted_in_subject += 1
            ...
            if not target_topic_name:
                print("✨ All threads processed inside current active layout view room.")
                if TARGET_URL not in knowledge["completed_subjects"]:
                    knowledge["completed_subjects"].append(TARGET_URL)
                save_knowledge_base(knowledge)
                return

            if uncompleted_in_subject <= 1:
                if TARGET_URL not in knowledge["completed_subjects"]:
                    print(f"🔒 Final assignment node claimed. Marking URL string as completely handled -> {TARGET_URL}")
                    knowledge["completed_subjects"].append(TARGET_URL)
